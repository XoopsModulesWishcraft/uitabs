 ...
 * @access 		public
 */
class UitabsFormSelectTab extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function UitabsFormSelectTab($caption, $name, $value = null, $size = 1, $multiple = false, $none = false)
    {
        ...
        if ($none==true)
        	$this->addOption('', _MI_TABS_NONE);
        $tabs_handler = xoops_getmodulehandler('tabs', 'uitabs');
		foreach($tabs_handler->getObjects(NULL, true) as $tid => $obj)
			$this->addOption($tid, $obj->getVar('name').' ('.$tid.')');
    }
