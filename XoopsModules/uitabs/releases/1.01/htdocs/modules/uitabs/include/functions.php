...
if (!function_exists("uitabs_checkpasskey")) {
	function uitabs_checkpasskey($key) {
		...
		$diff = intval(intval($GLOBALS['xoopsModuleConfig']['passkeyvalidfor'])*60);
		for($step=$minseed;$step<=($minseed+$diff);$step=$step+60) 
			if ($key==md5($GLOBALS['xoopsModuleConfig']['salt'].XOOPS_LICENSE_KEY.date('Ymdhi', $step)))
				return true;
		return false;
	}
}
...
if (!function_exists("uitabs_getPassKey")) {
	function uitabs_getPassKey() {
		return md5($GLOBALS['xoopsModuleConfig']['salt'].XOOPS_LICENSE_KEY.date('Ymdhi', time()+10));
	}
}