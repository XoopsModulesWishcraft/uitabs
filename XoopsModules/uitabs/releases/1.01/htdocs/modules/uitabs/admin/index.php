			...
			switch ($fct)
			{
				...
				case "list":				
					...
					foreach (array(	'iid','tid','title','summary','path','image','extension','weight','width','height','pid','fid','url','uid','rank','clicks','clicked','created','updated') as $id => $key) {
						$GLOBALS['utTpl']->assign(strtolower(str_replace('-','_',$key).'_th'), '<a href="'.$_SERVER['PHP_SELF'].'?start='.$start.'&limit='.$limit.'&sort='.str_replace('_','-',$key).'&order='.((str_replace('_','-',$key)==$sort)?($order=='DESC'?'ASC':'DESC'):$order).'&op='.$op.'&filter='.$filter.'">'.(defined('_AM_FLOWITEMS_TH_'.strtoupper(str_replace('-','_',$key)))?constant('_AM_FLOWITEMS_TH_'.strtoupper(str_replace('-','_',$key))):'_AM_FLOWITEMS_TH_'.strtoupper(str_replace('-','_',$key))).'</a>');
							...
							if ($uploader->fetchMedia('upload')) {
							  	if (!$uploader->upload()) {
									...
									exit(0);
							      	...
							      	$item->setVar('extension', $filename[sizeof($filename)]);
