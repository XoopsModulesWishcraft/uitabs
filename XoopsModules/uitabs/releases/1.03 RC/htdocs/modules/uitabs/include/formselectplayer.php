 ...
 * @access 		public
 */
class UitabsFormSelectPlayer extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function UitabsFormSelectPlayer($caption, $name, $value = null, $size = 1, $multiple = false, $none = false)
    {
        ...
        if ($none==true)
        	$this->addOption('', _MI_FLOWPLAYER_NONE);
        $player_handler = xoops_getmodulehandler('player', 'flowplayer');
		foreach($player_handler->getObjects(NULL, true) as $fid => $obj)
			$this->addOption($fid, $obj->getVar('name').' ('.$fid.')');
    }
