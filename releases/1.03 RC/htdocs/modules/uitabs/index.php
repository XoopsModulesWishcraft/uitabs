	...
	switch ($op) {
		...
		case "tabs":
			...
			if ($tabs_handler->getCount($criteria)>0) {
				...
				$GLOBALS['xoopsTpl']->assign('initialIndex', $initialIndex);
		...
		case "full":
			...
			include($GLOBALS['xoops']->path('header.php'));
			$GLOBALS['xoopsTpl']->assign('html', $item->toHT(false, false, 0 ,0));
			include($GLOBALS['xoops']->path('footer.php'));
			exit;
			break;			
		case "count":
			$items_handler->setClicked($id);
			redirect_header(($url!=$_SERVER['REQUEST_URI'])?$url:XOOPS_URL, 10);
			exit;
			break;
		case "vote":
